...
| 2-Plane Ingestion | Two-plane Bronze pipeline (planner → EC2 agent → raw_to_bronze) | ✅ COMPLETE, optimized & LOADED. **2026-07-10: 139 active instruments in bronze.market_data_daily (~430k bars) — core 60 (15yr) + expansion batches incl a high-ADV cohort with 8 recent IPOs (short history from IPO date, correct).** Perf+robustness hardening §3.5.1; Bronze table rebuilt with **Liquid Clustering** (was over-partitioned by bar_date). All on feature/phase3-silver. Perf numbers in memory raw_to_bronze_perf_findings. Full guide: docs/runbooks/ingestion_run_guide.md. Schedules still OFF (manual). Deferred ops: enable 7pm/8pm crons · PR feature/phase3-silver→main · drop bronze.market_data_daily_old_partitioned backup. |
| 3 | Silver (Feature Engineering) | 🔜 STARTING NEXT SESSION — step-by-step teaching track. **Handover: memory `phase3_kickoff_handover` (read first).** Lesson 1 (what is a feature) delivered; next = first hands-on EMA on real SPY (notebook-first, then formalize into src/silver ABCs). adjustment_factors is the gate only when generalizing to splitting stocks (SPY first — no splits). |
