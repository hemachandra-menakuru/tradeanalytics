...
from src.ingestion.jobs.bronze_ingestion_job import BronzeIngestionJob, JobRunSummary
from src.ingestion.writers.bronze_writer import BronzeWriter
...
@pytest.fixture
def job(config, mock_provider, writer, watermark_mgr, validator):
    """Build job with all components injected."""
    with patch(
        "src.ingestion.jobs.bronze_ingestion_job.MarketDataFactory"
        ".get_provider",
...
def test_provider_fallback_on_health_check_failure(config, writer, watermark_mgr, validator):
    ...
    with patch(
        "src.ingestion.jobs.bronze_ingestion_job.MarketDataFactory.get_provider",
        return_value=failing_provider
    ), patch(
        "src.ingestion.jobs.bronze_ingestion_job.MarketDataFactory.get_fallback_provider",
